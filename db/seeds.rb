...
alice = User.find_or_create_by!(email: "alice@example.com") do |u|
  ...
  u.last_name = "Martin"
  u.phone = 611234567
  u.phone_number = "611234567" if User.column_names.include?("phone_number")
...
bob = User.find_or_create_by!(email: "bob@example.com") do |u|
  ...
  u.last_name = "Chen"
  u.phone = 619876543
  u.phone_number = "619876543" if User.column_names.include?("phone_number")
...
carol = User.find_or_create_by!(email: "carol@example.com") do |u|
  ...
  u.last_name = "Park"
  u.phone = 612345678
  u.phone_number = "612345678" if User.column_names.include?("phone_number")
...
owner = User.find_or_create_by!(email: "owner@tapinstudio.com") do |u|
  ...
  u.last_name = "Lopez"
  u.phone = 610001111
  u.phone_number = "610001111" if User.column_names.include?("phone_number")
  u.confirmed_at = Time.current if u.respond_to?(:confirmed_at=)
end
...
demo_users = demo_members.map do |m|
  u = User.find_or_create_by!(email: m[:email]) do |user|
    ...
    user.phone = m[:phone]
    user.phone_number = m[:phone].to_s if User.column_names.include?("phone_number")
